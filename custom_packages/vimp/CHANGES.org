...
** News in 1.0.3
   ...
   * Fix `vimp-delete-backward-word` at first non-blank in a line.
...
*** Improvements
   ...
   * Bind =[tab]= like =\t= in =vimp-ex-completion-map=, so that it
     cannot be overwritten by a =[tab]= binding in the parent keymap
     =minibuffer-local-completion-map=.

   * Improve worst case performance of internal functions
     =vimp-in-string-p=, =vimp-string-beginning= and
     =vimp-string-end=. This functions are used, e.g., in certain text
...
*** Fixes
   ...
   * The interactive code =<state>= is fixed and used correctly in
     =vimp-ex-set-initial-state=.

   * =vimp-ex-global= always generates a single undo-step.
